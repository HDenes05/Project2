...
async def main():
    ...
    sky = pg.transform.smoothscale(pg.image.load(
        'skybox.jpg').convert(), (12*horizontal_res, vertical_res))
        ...
        for i in range(horizontal_res):  # vision loop
            ...
            while True:  # ray loop
                ...
                if mapa[int(x)][int(y)] != 0:
                    h = vertical_res * \
                        (1/(0.02 * n*math.cos(math.radians(i*mod-30))))
                    xx = x % 1
                    if xx < 0.05 or xx > 0.95:
                        xx = y % 1
                    resized = pg.transform.scale(wall, (h, h))
                    subsurface = pg.Surface.subsurface(
                        resized, (min(int(h)-1, int(xx*h)), 0, 1, int(h)))
...
def movement(posx, posy, rot, mapa, elapsed_time):
    ...
    forward = (pressed_keys[pg.K_UP] or pressed_keys[ord('w')]) - \
        (pressed_keys[pg.K_DOWN] or pressed_keys[ord('s')])
    sideways = (pressed_keys[pg.K_LEFT] or pressed_keys[ord(
        'a')]) - (pressed_keys[pg.K_RIGHT] or pressed_keys[ord('d')])
